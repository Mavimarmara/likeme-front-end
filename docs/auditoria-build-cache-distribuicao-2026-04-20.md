...
| Preview interno                              | `eas.json` → `build.preview` (estende `staging`, `distribution: internal`)                                                                                         | Herda `store-common`: `EXPO_SKIP_DISABLE_CAPABILITIES=1` (EAS não desliga capabilities remotas; ainda habilita as dos entitlements). Exceção total: `npm run build:ios:preview:skip-capability-sync`. |
| Staging (loja / AAB/APK conforme plataforma) | `eas.json` → `build.staging`                                                                          | `distribution: store`, Android **`apk`**, `autoIncrement`, env staging, `SKIP_IOS_POD_INSTALL`; herda `EXPO_SKIP_DISABLE_CAPABILITIES` via `store-common`.                                                                                           |
...
- [ ] Comando / perfil anotados, por exemplo:
  ...
  - Preview (interno, herda staging): `eas build --profile preview` (herda `EXPO_SKIP_DISABLE_CAPABILITIES`; só use `build:ios:preview:skip-capability-sync` para `EXPO_NO_CAPABILITY_SYNC=1` explícito)
