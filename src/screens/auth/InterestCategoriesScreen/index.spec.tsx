...
jest.mock('@/services', () => ({
  personCategoryService: {
    getMySelectedCategoryIds: jest.fn().mockResolvedValue([]),
    saveMyCategories: jest.fn().mockResolvedValue(undefined),
  },
  AuthService: {
...
jest.mock('@/hooks/interestCategories/useInterestCategories', () => {
  const actual = jest.requireActual<typeof import('@/hooks/interestCategories/useInterestCategories')>(
    '@/hooks/interestCategories/useInterestCategories',
  );
  return {
    ...actual,
...
describe('InterestCategoriesScreen', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    getServices().personCategoryService.getMySelectedCategoryIds.mockResolvedValue([]);
    getServices().personCategoryService.saveMyCategories.mockResolvedValue(undefined);
  ...
  it('navega para Home ao tocar Iniciar com ao menos uma categoria selecionada', async () => {
    ...
    await waitFor(() => {
      expect(getServices().personCategoryService.saveMyCategories).toHaveBeenCalledWith(['sleep']);
  ...
  it('navega para Home ao tocar Pular sem salvar', async () => {
    ...
    await waitFor(() => {
      expect(mockNavigation.navigate).toHaveBeenCalledWith('Home');
      expect(getServices().personCategoryService.saveMyCategories).not.toHaveBeenCalled();
    });
  });

  it('renderiza categorias em português após carregar', async () => {
    const mockNavigation = {
      navigate: jest.fn(),
      goBack: jest.fn(),
    };
  ...
  it('carrega categorias selecionadas do backend ao montar', async () => {
    const mockNavigation = { navigate: jest.fn(), goBack: jest.fn() };
    const mockRoute = { params: { firstName: 'John' } };

    render(<InterestCategoriesScreen navigation={mockNavigation} route={mockRoute as any} />);

    await waitFor(() => {
      expect(getServices().personCategoryService.getMySelectedCategoryIds).toHaveBeenCalled();
    });
  });

  it('restaura categorias já selecionadas ao carregar', async () => {
    getServices().personCategoryService.getMySelectedCategoryIds.mockResolvedValue(['sleep']);
  ...
  it('mostra alerta e não navega quando a API falha ao salvar', async () => {
    ...
    const mockRoute = { params: { firstName: 'John' } };
    const alertSpy = jest.spyOn(require('react-native').Alert, 'alert');
    getServices().personCategoryService.saveMyCategories.mockRejectedValue(new Error('API error'));
  ...
  it('não submete duas vezes enquanto o salvamento está em andamento', async () => {
    ...
    getServices().personCategoryService.saveMyCategories.mockReturnValue(submitPromise);
    ...
    await waitFor(() => {
      expect(getServices().personCategoryService.saveMyCategories).toHaveBeenCalledTimes(1);
    });
    expect(mockNavigation.navigate).not.toHaveBeenCalled();
  });
});
